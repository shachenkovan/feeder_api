...
def task_list_by_id(task_list_id: UUID, db: Session = Depends(get_db)):
    try:
...
def add_task_list(task_list: TaskListsSchemaPost, db: Session = Depends(get_db)):
    try:
...
def update_task_list_by_id(task_list_id: UUID, task_list: TaskListsSchemaUpdate, db: Session = Depends(get_db)):
    try:
...
def delete_task_list_by_id(task_list_id: UUID, db: Session = Depends(get_db)):
    try:
