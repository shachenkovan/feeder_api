...
def device_model_by_id(device_model_id: int, db: Session = Depends(get_db)):
    try:
...
def add_device_model(device_model: DeviceModelSchemaPost, db: Session = Depends(get_db)):
    try:
...
def update_device_model_by_id(device_model_id: int, device_model: DeviceModelSchemaPost, db: Session = Depends(get_db)):
    try:
...
def delete_device_model_by_id(device_model_id: int, db: Session = Depends(get_db)):
    try:
